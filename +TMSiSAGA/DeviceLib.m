classdef DeviceLib < TMSiSAGA.HiddenHandle
    ...
    methods (Static = true)
        ...
        function device_list = getDeviceList(ds_interface_type, dr_interface_type, maxNumOfRetries, max_num_devices)
            ...
            % the device.
            nrOfRetries = 1;
            if exist('maxNumOfRetries', 'var')
                if (maxNumOfRetries > 0)
                    nrOfRetries = maxNumOfRetries; 
                end
            end
            ...
            while nrOfRetries > 0
                ...
                % Once the device is found, break from the while loop.
                if (status == 0) 
                    nrOfRetries = 0;
                else
        ...
        function [device_report, battery_report_list, ...
            ...
            %GETFULLDEVICESTATUS - Get a full report from the device status
            %   including battery, time and storage information.
        ...
        function [recorder_configuration, channel_list] = getDeviceConfig(handle, num_channels)
            %GETDEVICECONFIG - Retrieve the configuration currently in use
            %   by the device
        ...
        function setDeviceSampling(handle, deviceSamplingMode)
            %SETDEVICESAMPLING - Send a request to the device to start
            %   sampling.
        ...
        function setDeviceImpedance(handle, deviceImpedanceMode)
            %SETDEVICEIMPEDANCE - Send a request to the device to start
            %   sampling in impedance mode.
        ...
        function sensors = getDeviceSensors(handle, num_sensors)
            %GETDEVICESENSORS - Obtain a list from the device that states 
            %   which channels are sensor channels.
            if TMSiSAGA.DeviceLib.trace()
            ...
            % Extract information from the sensor_list pointer.
        ...
        function resetDeviceDataBuffer(handle)
            %RESETDEVICEDATABUFFER - Reeet the data buffer of the device
            if TMSiSAGA.DeviceLib.trace()
        ...
        function [data, num_sets] = getDeviceRepairData(handle, buffer, buffer_len, repair_request)
            %GETDEVICEREPAIRDATA - Send the repair request to the recording
            %   on the device to ensure missing data can be repaired. 
