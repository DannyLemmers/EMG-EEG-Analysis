classdef ImpedancePlot < TMSiSAGA.HiddenHandle
   ...
   properties
    ...
    methods
        function obj = ImpedancePlot(name, enabled_channels, channel_names, SAGA_type)
            %IMPEDANCEPLOT - Constructor function for the ImpedancePlot Class
            %
            %   obj = ImpedancePlot(name, enabled_channels, channel_names, SAGA_type)
            %
            %   obj [out] - ImpedancePlot object.
            %   name [in] - Name of the figure window.
            %   enabled_channels [in] - Channels that have been enabled for 
            %       the measurement
            %   channel_names [in] - Configured names for the impedance
            %       measurement
        ...
        function show(obj)
            %SHOW - Function that creates the figure window that shows the
            %   impedance
            %
            %   show(obj)
            %
            ...
            % To avoid opening a new window when the plot is closed, only
            % execute this if the object is visible
            if obj.is_visible
                set(obj.figure, 'Units', 'Normalized', 'OuterPosition', [0, 0, 1, 1]);
        ...
        function head_layout(obj, impedance, ChanLocs)
            ...
            hold on
            plot(nan,nan,'o','MarkerSize',15,'MarkerEdgeColor','k','MarkerFaceColor',...
            ...
            legend('AutoUpdate','off','Location','northeastoutside')
            legend('boxoff')
            ...
            for i = 1:numel(ChanLocs)
                plot(ChanLocs(i).radius * sind(ChanLocs(i).theta), ...
                ...
                if obj.num_channels == 32
                    text(ChanLocs(i).radius  * sind(ChanLocs(i).theta) , ...
            ...
            text(-0.075,-0.55,['PGND impedance: ' num2str(impedance(end,end)) 'k\Omega'], 'Color', 'k');
            text(-0.075,-0.575,['CREF impedance: ' num2str(impedance(1,end)) 'k\Omega'], 'Color', 'k');
    ...
    methods(Access = private)
        ...
        function CleanUpFigure(obj)
            ...
            if isempty(obj.basic_children)
                obj.basic_children = allchild(gca);
            end
            
            % Get the current graphical objects
            obj.children = allchild(gca);
            ...
            obj.previous_children = allchild(gca);
        end
        ...
        function ImpedanceList(obj, impedance, flag)
            ...
            if strcmp(flag,'EEG')
                ...
                for ii = 2:size(impedance,1)-1
                    ...
                    if obj.num_channels == 64
                        if ii <= 33
                            text_imp = text(-0.96,0.55-ii/32,[obj.channel_names{ii} ...
                                ':  ' num2str(impedance(ii,end)) 'k\Omega']);                            
                        elseif ii> 33 
                            text_imp = text(-0.76,0.55-(ii-32)/32,[obj.channel_names{ii} ...
                                ':  ' num2str(impedance(ii,end)) 'k\Omega']);                            
                        end
                    elseif obj.num_channels == 32
                        text_imp = text(-0.76,0.55-ii/32,[obj.channel_names{ii} ...
                            ':  ' num2str(impedance(ii,end)) 'k\Omega']);
                    end
                end
            ...
            elseif strcmp(flag,'HDEMG')
                ...
                for ii = 2:size(impedance,1)-1
                    ...
                    if obj.num_channels == 64
                        if ii <= 33
                            text_imp = text(-2.3, 0.3 + ii/4,[obj.channel_names{ii} ...
                                ':  ' num2str(impedance(ii,end)) 'k\Omega']);
                        elseif ii > 33 && obj.num_channels > 32
                            text_imp = text(-0.9, 0.3 + (ii-32)/4,[obj.channel_names{ii} ...
                                ':  ' num2str(impedance(ii,end)) 'k\Omega']);
                        end
                    elseif obj.num_channels == 32
                        if ii <= 33
                            text_imp = text(0, 0.25 + ii/8,[obj.channel_names{ii} ...
                                ':  ' num2str(impedance(ii,end)) 'k\Omega']);
                        end
                    end
                end
        ...
        function CreateGridEMG(obj, grid_values, impedance) 
            ...
            % Create legend entries
            plot(nan,nan,'o','MarkerSize',20,'MarkerEdgeColor','k','MarkerFaceColor',...
                obj.impedance_colormap(1,:),'DisplayName','1-5 k\Omega')
            plot(nan,nan,'o','MarkerSize',20,'MarkerEdgeColor','k','MarkerFaceColor',...
                obj.impedance_colormap(2,:),'DisplayName','5-10 k\Omega')
            plot(nan,nan,'o','MarkerSize',20,'MarkerEdgeColor','k','MarkerFaceColor',...
            ...
            % show the user this in the figure window
            if obj.num_channels == 64
                text(0.9, 8.5, ['CREF impedance: ' num2str(impedance(1,end)) 'k\Omega']);
                text(7.0, 8.5, ['PGND impedance: ' num2str(impedance(end,end)) 'k\Omega']);
            elseif obj.num_channels == 32
                text(0.9, 4.7, ['CREF impedance: ' num2str(impedance(1,end)) 'k\Omega']);
                text(7.0, 4.7, ['PGND impedance: ' num2str(impedance(end,end)) 'k\Omega']);
            end
            ...
            for ii = 1:obj.num_channels
                if mod(ii,8) == 1; r = r+1; c = 1; 
                    txt_locs = text(c,r,2000,[obj.channel_names{ii+1}],...
                        'HorizontalAlignment', 'center', 'VerticalAlignment', 'middle');
                else   
                    c = c+1; text(c,r,2000,[obj.channel_names{ii+1}], ...
                        'HorizontalAlignment', 'center', 'VerticalAlignment', 'middle');  
                end
            end
